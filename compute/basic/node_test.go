...
func TestNodeTimeout(t *testing.T) {
	conf := config.DefaultConfig()
	conf.Backends.Basic.Node.Timeout = time.Millisecond
	conf.Backends.Basic.Node.UpdateRate = time.Millisecond * 2
	...
	// Fail if this test doesn't complete in the given time.
	cleanup := timeLimit(t, conf.Backends.Basic.Node.Timeout*500)
	defer cleanup()
