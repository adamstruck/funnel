...
func TestGetMetadata(t *testing.T) {
	...
	if conf.Backends.Basic.Backends.GCE.Project != "funnel-proj-id" {
		t.Fatal("Unexpected project id")
	}

	if conf.Backends.Basic.Backend != "gce" {
		t.Fatal("Unexpected scheduler")
	}

	// When meta.instance.attributes.funnelNode != ""
	// conf.Backends.Basic.Node.ID == meta.instance.name
	if conf.Backends.Basic.Node.ID != "funnel-node-1492486244" {
		t.Fatal("Unexpected node ID")
	}

	if conf.Backends.Basic.Node.Metadata["gce"] != "yes" {
		t.Fatal("Expected gce tag in node metadata")
	}

	if conf.Backends.Basic.Backends.GCE.Zone != "us-west1-b" {
