...
import (
	"errors"
	gce_mocks "github.com/ohsu-comp-bio/funnel/compute/basic/gce/mocks"
...
func TestInsertTempError(t *testing.T) {
	...
	conf.Backends.Basic.Node.ID = "test-node"
	...
	client.StartNode("test-tpl", conf.Backends.Basic.Node.ServerAddress, conf.Backends.Basic.Node.ID)
	client.StartNode("test-tpl", conf.Backends.Basic.Node.ServerAddress, conf.Backends.Basic.Node.ID)
	client.StartNode("test-tpl", conf.Backends.Basic.Node.ServerAddress, conf.Backends.Basic.Node.ID)
