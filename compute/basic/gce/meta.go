...
func WithMetadataConfig(conf config.Config, meta *Metadata) (config.Config, error) {
	...
	if meta.Instance.Attributes.FunnelNodeServerAddress != "" {
		...
		conf.Backends.Basic.Node.ServerAddress = meta.Instance.Attributes.FunnelNodeServerAddress
		conf.Worker.ServerAddress = meta.Instance.Attributes.FunnelNodeServerAddress
	}

	if meta.Project.ProjectID != "" {
		conf.Backends.Basic.Backends.GCE.Project = meta.Project.ProjectID
	}
	...
	if meta.Instance.Zone != "" {
		...
		conf.Backends.Basic.Backends.GCE.Zone = zone
	}
	...
	if conf.Backends.Basic.Node.ID != "" {
		if conf.Backends.Basic.Node.ServerAddress == "" {
