...
func NewNode(conf config.Config) (*Node, error) {
	log := logger.Sub("node", "nodeID", conf.Backends.Basic.Node.ID)

	cli, err := NewClient(conf.Backends.Basic)
	if err != nil {
		return nil, err
	}

	err = util.EnsureDir(conf.Backends.Basic.Node.WorkDir)
	if err != nil {
		return nil, err
	}

	// Detect available resources at startup
	res := detectResources(conf.Backends.Basic.Node)
	timeout := util.NewIdleTimeout(conf.Backends.Basic.Node.Timeout)
	state := pbs.NodeState_UNINITIALIZED

	workerConf := conf.Worker
	workerConf.WorkDir = conf.Backends.Basic.Node.WorkDir

	log.Debug("config", "node", conf.Backends.Basic.Node, "worker", workerConf)

	return &Node{
		conf:       conf.Backends.Basic.Node,
