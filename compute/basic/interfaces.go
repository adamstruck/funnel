...
// SchedulerBackend is responsible for scheduling a task. It has a single method which
...
// Various backends (Openstack, GCE, etc.) implement this, which
// allows BackendLoader to load a backend by name (e.g. config.Backend.Basic.Backend).
type BackendFactory func(config.Config) (SchedulerBackend, error)

// BackendLoader helps load a funnel basic scheduler backend by name
// (e.g. config.Backend.Basic.Backend).
type BackendLoader map[string]BackendFactory

// Load finds a funnel basic scheduler backend by name and returns a new instance.
func (bl BackendLoader) Load(name string, conf config.Config) (SchedulerBackend, error) {
	name = strings.ToLower(name)
	factory, ok := bl[name]

	if !ok {
		log.Error("Unknown backend", "name", name)
